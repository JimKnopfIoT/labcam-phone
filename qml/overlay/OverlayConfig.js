...
// ============================================================
//  LCR formatting (DE-5000). Value arrives in base SI (F/H/Ω); picks the
//  matching engineering prefix (p/n/µ/m/k/M). D/Q/θ are unitless resp. degrees.
//  Returns {num, unit}.
// ============================================================
function fmtLcr(value, unit) {
    if (value === null || value === undefined || !isFinite(value))
        return { num: "—", unit: unit || "" }
    if (unit === "" || unit === "%" || unit === "°") {   // D / Q / θ — no prefix
        var a0 = Math.abs(value)
        return { num: value.toFixed(a0 >= 100 ? 1 : (a0 >= 10 ? 2 : 4)), unit: unit || "" }
    }
    var PFX = [
        { p: 1e-12, s: "p" }, { p: 1e-9, s: "n" }, { p: 1e-6, s: "µ" },
        { p: 1e-3,  s: "m" }, { p: 1,    s: ""  }, { p: 1e3,  s: "k" }, { p: 1e6, s: "M" }
    ]
    var abs = Math.abs(value)
    var chosen = PFX[4]
    for (var i = 0; i < PFX.length; i++) {
        var scaled = abs / PFX[i].p
        if (scaled >= 1 && scaled < 1000) { chosen = PFX[i]; break }
    }
    var num = value / chosen.p
    var an = Math.abs(num)
    return { num: num.toFixed(an >= 100 ? 2 : (an >= 10 ? 3 : 4)), unit: chosen.s + unit }
}

// Auto Y-maximum for the LCR graph (value ranges over many decades).
function lcrYMax(value) {
    if (value === null || value === undefined || !isFinite(value)) return 1
    var abs = Math.abs(value)
    if (abs < 1e-15) return 1
