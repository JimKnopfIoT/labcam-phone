...
Item {
    ...
    property var lcr:  ({ ok: false, source: "cp2102", status: "—", mode: "—", value: null,
                          unit: "", sec_mode: "", sec_value: null, sec_unit: "", freq: null })
    ...
    function _applyState(state) {
        ...
        if (d.lcr)  root.lcr  = d.lcr
        if (state.ts) root.lastTs = state.ts
        root.stateUpdated()
    }

    // Send a command to the backend (back-channel), e.g. toggle the DE-5000 source:
    //   sendCommand({ cmd: "lcr_source", value: "toggle" })
    function sendCommand(obj) {
        if (socket.status === WebSocket.Open) {
            socket.sendTextMessage(JSON.stringify(obj))
            return true
        }
        console.warn("[instr] sendCommand dropped (socket not open)")
        return false
    }
